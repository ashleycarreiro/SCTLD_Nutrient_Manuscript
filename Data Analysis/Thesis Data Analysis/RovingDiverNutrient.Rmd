...
```{r, load, include = "true"}
PD <- read.xls("RovingDiverNutrientData/NutrientRovingDiver.xlsx")
PD
PD$Date <- ymd(PD$Date)
...
#adds intercept lint if not number have to do as.factor or whatever it might be
  geom_text(aes(x=as.factor("2021-10-22"), label="End Treatment", y=13.3), colour="black",angle=90, vjust=2.5)+
...
ggsave("Figures/Figure9_SCTLDPrevalence.LineGraph.png", plot = SCTLDPrevalence.LineGraph, height = 5, width = 9, units = 'in', dpi = 600)
```
...
ggsave("Figures/Figure10_SCTLDPrevalence.Boxplot.png", plot = SCTLDPrevalence.Boxplot, height = 5, width = 9, units = 'in', dpi = 600)
```
...
```{r}
ggscatter(ActiveStatusData, x= "SCTLDPrevalence", y="Temp", add= "reg.line", conf.int=TRUE, cor.coef=TRUE, cor.method="pearson")
```
 
```{r}
ActiveStatusData
cor.test(formula = ~SCTLDPrevalence + Temp, data = ActiveStatusData)
