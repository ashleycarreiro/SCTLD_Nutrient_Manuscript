...
```{r, load, include = "true"}
WaterData <- read.xls("WaterQualityOnlyData/WaterQualityDataOnly.xlsx")
OrderedNutrientData<-read.xls("WaterQualityOnlyData/NutrientData.xlsx")
RainData <- read.xls("WaterQualityOnlyData/RainData.xlsx")
NewWaterData <- read.xls("WaterQualityOnlyData/NewNutrientData.xlsx")
NewWaterDataDIN <- read.xls("WaterQualityOnlyData/NewNutrientDataHorizontal.xlsx")
...
```{r}
Exp.TreatmentWaterData %>%
  group_by(DaysSinceNutrientDeployment) %>%
  get_summary_stats(DIN.SRP, show= c("min", "max", "mean"))
```
...
```{r}
FilteredWaterData$DaysSinceNutrientDeployment <- factor(FilteredWaterData$DaysSinceNutrientDeployment, levels = c("Pre-Amendment", "1hr", "2Day", "3Day", "5Day", "7Day"))
levels(FilteredWaterData$DaysSinceNutrientDeployment)
```

```{r}
DIN.bxp.Filtered <- ggboxplot(FilteredWaterData, x="DaysSinceNutrientDeployment", y="DIN", color="Group") +
...
#Check DIN.SRP Values for Normalicy All data is normal except for two of the timepoints between groups
...
```{r}
TreatmentDays.Kruskal <- NutrientWaterData %>% kruskal_test(DIN.SRP ~ DaysSinceNutrientDeployment)
TreatmentDays.Kruskal

```

```{r}
NH4TreatmentDays.Kruskal <- NutrientWaterData %>% kruskal_test(NH4 ~ DaysSinceNutrientDeployment)
...
```{r}
Nutrient.bxp <-  ggboxplot(NutrientWaterData, x="DaysSinceNutrientDeployment", y="DIN.SRP")
Nutrient.bxp
```

```{r}
Nutrient.bxp1 <-  ggboxplot(NutrientWaterData, x="DaysSinceNutrientDeployment", y="NO23")
Nutrient.bxp1
```

```{r}
Nutrient.bxp2 <-  ggboxplot(NutrientWaterData, x="DaysSinceNutrientDeployment", y="NH4")
Nutrient.bxp2
```
...
Nutrients.Summary <- summarySE(HealthyWaterData, measurevar = "DIN.SRP", groupvars = c("Date"), na.rm=TRUE) 
...
```{r}
DIN.SRP.LineGraph<-  ggplot(data=Nutrients.Summary, aes(x=(Date),y=DIN.SRP, color='character')) +
  geom_line( group='character', color="#117733", size=1.5)+
# had to add this grouping so i could change the color
  geom_errorbar(aes(ymin=DIN.SRP-se, ymax=DIN.SRP+se), color= "#117733", size=1.5, width=0.5) +
 ...
 geom_vline(xintercept= as.factor("2021-09-24"), linetype=4, color="black")+
  geom_text(aes(x=as.factor("2021-09-24"), label="Start Treatment", y=20), colour="black",angle=90, vjust=2) +
  


  geom_vline(xintercept= as.factor("2021-10-22"), linetype=4, color="black")+
geom_text(aes(x=as.factor("2021-10-22"), label="End Treatment", y=20), colour="black",angle=90, vjust=1.5) +
...
#geom_bar(stat="identity", position="dodge")+
  ...
  geom_errorbar(aes(ymin=Measurement-se, ymax=Measurement+se, group=Group), width =0.2) + 
   ...
   facet_wrap(Nutrient~., scales="free_y", strip.position="right", ncol=1) +
    geom_vline(xintercept= as.factor("2021-10-22"), linetype=4, color="black")+
   #geom_text(aes(x=as.factor("2021-10-22"), label="End Treatment", y=1), colour="black",angle=90, vjust=2.5)+
 # geom_vline(xintercept= as.factor("2021-09-24"), linetype=4, color="black")+
...
ggsave("Figures/Figure4_AllNutrients.LineGraph.JPG", plot =AllNutrient.LineGraph, height = 9, width = 9, units = 'in', dpi = 600)
```
...
```{r}
shapiro_test(RainData1, meanRain)
```
...
```{r}
cor.test(RainData1$meanRain, RainData1$meanDIN, method="pearson")
```
...
```{r}
ggplot(RainData1, aes(x=Date, y=meanRain, group=1)) +
  geom_line()
```
...
ggsave("Figures/Figure5_NewAllNutrients.LineGraph.JPG", plot =NewNutrients.LineGraph, height = 6, width = 11, units = 'in', dpi = 600)
```
```{r}
...
scale_x_discrete(limits=c("2023-03-05", "2023-03-06","2023-03-08", "2023-03-10"), labels=c("1 hr","1 day", "2 days","4 days")) +
   labs(y="Trial 2 Nutrient Concentrations log(µM)", x ="Timepoint")  +
...
ggsave("Figures/SuppFigure2_NewAllNutrients.Log.LineGraph.JPG", plot =NewNutrients.Log_LineGraph, height = 6, width = 11, units = 'in', dpi = 600)
```
...
```{r}
NewWaterDataDIN
NewWaterDataDIN$Group <- factor(NewWaterDataDIN$Group, levels=c("Nutrient Ammended", "Control"))
levels(NewWaterDataDIN$Group)
NewWaterDataDIN
```
...
```{r}
FilteredOutliersNew %>% group_by(Date) %>%
  emmeans_test(NH4~Group, p.adjust.method = "bonferroni")
```
